...
{
    public partial class WHOrder : Form
    {
        private int RowIndex = -1;

        public WHOrder(int rowIndex)
        {
            InitializeComponent();
            if (rowIndex >= 0)
            {
                RowIndex = rowIndex;
                LoadData(WH.GetCurrentDepartment().OrderList.Rows[rowIndex]);
            }
        }
        ...
        private void btnConfirm_Click(object sender, EventArgs e)
        {
            ...
            var department = WH.GetCurrentDepartment();
            if (department != null)
            {
